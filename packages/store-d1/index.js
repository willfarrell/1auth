...
// Cloudflare D1 storage. The SQL is the same as @1auth/store-sql generates, but
// D1 binds prepared statements rather than exposing `query(sql, parameters)`,
// so the operations are its own.
...
const bind = (sql, parameters) =>
	options.client.prepare(sql).bind(...parameters);
...
export const exists = async (table, filters) => {
	...
	const res = await bind(sql, parameters).first();
	return res?.sub;
};
...
export const select = async (table, filters = {}, fields = []) => {
	...
	const sql = `SELECT ${select} FROM ${table} ${where} LIMIT 1`;
	const row = await bind(sql, parameters).first();
...
export const selectList = async (table, filters = {}, fields = []) => {
	...
	const res = await bind(sql, parameters).all();
	// Workaround because an expire filter doesn't exist yet
